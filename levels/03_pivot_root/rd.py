...
def contain(command, image_name, image_dir, container_id, container_dir):
    ...
    old_root_dirname = 'old-root'
    old_root_inside_path = os.path.join('/', old_root_dirname)
    old_root_outside_path = os.path.join(new_root, old_root_dirname)
    os.makedirs(old_root_outside_path)

    linux.pivot_root(new_root, old_root_outside_path) 
    os.chdir('/')

    # umount2 old root (HINT: see MNT_DETACH in man mount)
    linux.umount2(os.path.join('/', old_root_inside_path), linux.MNT_DETACH)
    os.rmdir(old_root_inside_path)
